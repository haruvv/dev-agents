...
**すべてのエージェント定義ファイル（`.claude/agents/*.md`）は冒頭に以下を記載し、共通ルールを明示的に取り込むこと：**

```
<!-- 共通ルール: .claude/agents/common.md を参照 -->
```

共通ルールの内容（`blocked` チェック・`in-*` ラベル管理・失敗時処理・`docs/` 参照）は `.claude/agents/common.md` で定義している。
各エージェントはこの共通ルールに従った上で、個別の処理を実行する。
