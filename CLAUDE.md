...
コミットメッセージ形式：`<type>: <変更内容の要約>`  
type は `feat` / `fix` / `docs` / `refactor` / `test` / `chore` のいずれか。

ファイル命名・ドキュメントフォーマットの詳細 → `docs/README.md`
...
技術スタックが決まったら `scripts/check-*.sh` と `ci.yml` の TODO を埋めること。
